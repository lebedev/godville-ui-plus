...
*/
	...
	var getOverlay = function () {
		if (!overlay) {
			overlay = tag('div', {id: Date.now().toString(32)});
		}
		if (!overlay.parentNode) {
			document.body.appendChild(overlay);
		}
		if (overlay.style.display === 'none' || overlay.style.visibility === 'hidden') {
			overlay.setAttribute('style', 'display:inline!important;visibility:visible!important;');
		}
		return overlay;
	};
...
function startPonies() {
	...
	BrowserPonies.loadConfig(
		{
			...
			"ponies":
				[{
					"baseurl":"http://zeird.github.io/godville-ui-plus/images/",
					"behaviors":
	...
	var newParasprite, clearParasprite = setInterval(function() {
		if (BrowserPonies.ponies().parasprite.instances.length && BrowserPonies.ponies().parasprite.instances[0].current_behavior && BrowserPonies.ponies().parasprite.instances[0].current_behavior.name === 'eat') {
			...
			newParasprite = setTimeout(function() {
				BrowserPonies.spawn('parasprite', 1);
			}, (10 + Math.floor(Math.random()*50))*1000);
		}
	}, 100);
	var poniesEnd = setInterval(function() {
		var curDate = new Date();
		if (curDate.getMonth() === 3 && curDate.getDate() === 1) {
			return;
		}
		clearInterval(poniesEnd);
		clearTimeout(newParasprite);
		clearInterval(clearParasprite);
		BrowserPonies.unspawnAll();
	}, 60*1000);
}
var poniesStart = setInterval(startPonies, 60*1000);
startPonies();