(function() {
	...
	if (location.href.match('boss') ||
		!document.getElementById('fight_log_capt').textContent.match('Хроника подземелья') ||
		+document.getElementById('fight_log_capt').textContent.match(/Хроника подземелья \(шаг (\d+)\)/)[1] < (localStorage[godname_prefix + 'LEMRestrictions:FirstRequest'] || 12)) {
		return;
	}
