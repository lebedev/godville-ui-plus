...
var cmd = path.resolve(__dirname + '../../../bin/growlnotify/growlnotify.COM');
...
function createMessageArg(message) {
  return [macOnly('-m') + message];
}

function createCallbackURLArg(callbackurl) {
  if (callbackurl) {
    return ['/cu:' + callbackurl];
  }
  return [];
}
...
function notify(options, cb) {
  ...
  var args = []
      ...
      .concat(createCallbackURLArg(options.callbackurl));
